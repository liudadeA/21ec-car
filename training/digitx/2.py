...
CIRCLE_SEND_INTERVAL = 0.4  # 圆上点发送间隔（秒）
...
class TargetTracker:
    def __init__(self):
        ...
        # 圆上点发送相关参数
        self.current_circle_idx = 0  # 当前发送的点索引
        self.last_send_time = 0.0    # 上次发送时间
        self.circle_points = []      # 存储圆上点（正视图像坐标）
        self.proj_points = []        # 存储投影到原始图像的点
    ...
    def update_circle_points(self, circle_pts, proj_pts):
        """更新圆上点列表"""
        self.circle_points = circle_pts
        self.proj_points = proj_pts

    def send_next_circle_point(self, uart):
        """检查并发送下一个圆上点"""
        if not self.is_truly_locked() or len(self.proj_points) < CIRCLE_POINTS:
            return

        current_time = time.time()
        if current_time - self.last_send_time >= CIRCLE_SEND_INTERVAL:
            # 获取当前索引对应的点（循环发送）
            idx = self.current_circle_idx % CIRCLE_POINTS
            point = self.proj_points[idx]
            
            # 发送格式：\xAA + P + 索引 + , + x + , + y + \x55\x44\x33
            send_data = f"\xAA P{idx},{point[0]:03d},{point[1]:03d}\x55\x44\x33"
            try:
                uart.write(send_data.encode())
                print(f"发送圆上点 {idx}: ({point[0]}, {point[1]})")
            except Exception as e:
                print(f"圆上点发送失败: {e}")
            
            # 更新索引和时间
            self.current_circle_idx += 1
...
def generate_circle_points(center, radius, num_points=36):
    """生成圆上的多个点，以最左侧为第一个点（270度），逆时针编号"""
    points = []
    start_angle = math.pi * 1.5  # 270度（最左侧）
    angle_step = 2 * math.pi / num_points  # 每个点的角度间隔
...
def draw_to_canvas(canvas, show_img, processed_img, tracker, detected_rects, threshold, auto_thresh, fps):
    ...
    mode_text = "模式: 找中心" if operation_mode == 0 else "模式: 单应性变换(画圆)"
    cv2.putText(canvas, mode_text, (SLIDER_X-90, SLIDER_Y + SLIDER_HEIGHT + 55), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

    # 10. 绘制圆上点编号（仅画圆模式）
    if operation_mode == 1 and tracker.is_truly_locked() and len(tracker.proj_points) >= CIRCLE_POINTS:
        for i, point in enumerate(tracker.proj_points[:CIRCLE_POINTS]):
            # 在原始图像上绘制点编号
            cv2.putText(canvas, f"{i}", (point[0]+5, point[1]+5),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
            # 在中间处理图上绘制
            cv2.putText(canvas, f"{i}", (point[0]+PICTURE_WIDTH+5, point[1]+5),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
        
        # 标记当前发送的点
        current_idx = tracker.current_circle_idx % CIRCLE_POINTS
        current_point = tracker.proj_points[current_idx]
        cv2.circle(canvas, current_point, 4, (0, 0, 255), -1)  # 红色标记当前发送点
        cv2.circle(canvas, (current_point[0]+PICTURE_WIDTH, current_point[1]), 4, (0, 0, 255), -1)
...
def draw_status_info(canvas, tracker, fps, auto_thresh):
    ...
    # 圆上点发送状态（仅画圆模式）
    if operation_mode == 1 and tracker.locked:
        send_status = f"发送点: {tracker.current_circle_idx % CIRCLE_POINTS}"
        cv2.putText(canvas, send_status, (SLIDER_X-75, SLIDER_Y - 85), 
...
def draw_tracking_status(canvas, tracker, original_img):
    ...
    # 模式1: 单应性变换模式（画圆模式）
        ...
        if transformed_img is not None and h_inv is not None:
            ...
            # 保存投影后的点（用于发送）
            tracker.update_circle_points(circle_points, projected_points)
...
def send_uart_data(u1, tracker, fps):
    """发送UART数据（中心坐标）"""
    if u1 is None or not tracker.locked or operation_mode == 1:
        return  # 画圆模式下不发送中心坐标，避免冲突
    ...
    try:
        ...
        print(f"发送中心坐标: {aaa},{bbb}, FPS: {fps:.1f}, 模式: {operation_mode}")
    except Exception as e:
        print(f"串口发送失败: {e}")
...
def main():
    ...
    try:
        while True:
            ...
            # 画圆模式下发送圆上点
            if operation_mode == 1 and u1 is not None:
                tracker.send_next_circle_point(u1)
            ...
            if key == ord('q'):
                ...
                mode_name = "找中心" if operation_mode == 0 else "单应性变换(画圆)"
                print(f"操作模式已切换为: {mode_name} (模式{operation_mode})")
                tracker.reset()  # 切换模式时重置跟踪器
...
def print_help():
    ...
    print("h: 显示帮助")
    print("d: 切换本地显示开启/关闭")
    print("m: 切换操作模式 (0-找中心, 1-单应性变换(画圆))")
